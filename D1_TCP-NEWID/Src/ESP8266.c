...
{
	STATUS_AP_CONNECTED,
	STATUS_TCP_UDP_CONNECTED,
	STATUS_CONNECTION_DROPPED,
	STATUS_AP_NOT_CONNECT,
}esp8266_local_status_t;
...
uint8_t receiveData(char* str)
{
	...
	if(strchr(str,'{'))
	{
		char* kuohao = strchr(str,'{');
		char* kuohao2 = strrchr(str,'}');
		if(kuohao && kuohao2)
		{
			uint16_t len = kuohao2-kuohao+2;
			g_esp8266_receiveString = malloc(len);
			if (g_esp8266_receiveString)
			{
				strncpy(g_esp8266_receiveString,kuohao,len);
			}
			return 1;
		}
	}
...
//			if (g_esp8266_receiveString)
//			{
//				strncpy(g_esp8266_receiveString,maohao,len);
//			}
...
//	{
//		volatile char *strA = str;
//		char* gmt = strstr(str,"GMT")+5;
//		char* douhao = strstr(str,"+IPD,0,") + 6;
//		char* maohao = strchr(gmt,'{');
//		if (douhao && maohao)
//		{
//			*douhao = 0;
//			*maohao = 0;
...
void esp8266_connectTCP(char* address, uint16_t port)
{
	...
	setCommandWait(1000,"CIPMUX","1",NULL); 
	setCommandWait(5000,"CIPSTART","0","\"TCP\"",addressForAT,buffer,"120",NULL);
	free(addressForAT);
}

uint8_t esp8266_tcp_keep_connect(char* address, uint16_t port)
{
	if (!g_esp8266_smartStarted)
	{
		...
		{
			esp8266_connectTCP(address,port);
		}
	}
}

uint8_t esp8266_tcp(char * str, char* address, uint16_t port)
{
...
//		}
		if (status == 3)
		{
			...
			"Accept: application/json\r\n"		
			"\r\n";
			sprintf(data,head,"/readings",length, address);
			
//			esp8266_sendHTTPRequest("/", str);
			char buffer[0x10];
			sprintf(buffer,"%d",strlen(data)+strlen(str));
			setCommandWait(0,"CIPSENDEX","0",buffer,NULL);
			if(waitUntilOk(500))
			{
				printf(data);
				printf(str);
